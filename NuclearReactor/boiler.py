...
def boiler(height):
    ...
    while heat < 101:
        ...
        for i in range(5):
            console.print(' '*4, '[yellow]'+block+'[/yellow]', ' ', '[yellow]'+block+'[/yellow]', ' '*5)
        console.print('[yellow]' + block*6 + '[/yellow]' + ' '*3 + '[yellow]' + block*5 + '[/yellow]')
        r_value = int(heat*255/100)
        for i in range(height+3):
            if i < 3:
                console.print('[yellow]'+block+'[/yellow]', f'[rgb(255,255,255)]' + block*10 + f'[/rgb(255,255,255)]', '[yellow]'+block+'[/yellow]')
            else:
